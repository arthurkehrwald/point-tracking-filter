...
class SplineParams:
    ...
    ``smoothing`` is the scipy smoothing condition per sample; the effective
    value handed to the spline is ``smoothing * n_samples`` so it stays
    comparable across recordings of different length.
    """
...
def apply_spline_filter(track: Track, params: SplineParams | None = None) -> Track:
    ...
    for indices in track.segments():
        ...
        if seg_t.size < max(params.degree + 1, MIN_SEGMENT_SAMPLES):
            kept.append(indices)
            times.append(seg_t)
