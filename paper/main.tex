...
The smoothing parameter is set manually to $\lambda = 10^{-4}~\text{s}^3$ instead of being chosen by GCV\@.
...
The windowed spline uses the manually set $\lambda = 10^{-4}~\text{s}^3$ and $W = 300$~ms for all recordings.
...
The RMSE decreases by 24\% without an additional horizon (0.84 to 0.64~cm), by 50\% at $h = 50$~ms (1.61 to 0.80~cm), and by 52\% at $h = 100$~ms (2.47 to 1.18~cm).
At $h = 50$~ms, the prediction is thus more accurate than the hold without any additional latency.
At $h = 0$, the RMSE of 0.64~cm hardly exceeds the mean offline deviation of the raw data of 0.60~cm (\cref{tab:filtering}):
the remaining error is essentially the systematic triangulation error discussed above, which no predictor can remove.
Its output is also more than twice as calm as that of the hold (jerk ratio 3.8 versus 9.0 at $h = 0$), although at $h = 100$~ms it becomes jerkier than the hold (15.8 versus 8.9).
...
The windowed spline reduces the RMSE by 25\% at $h = 0$ (4.24 to 3.16~cm) and by 19\% at $h = 50$~ms (9.98 to 8.09~cm), and at $h = 100$~ms it is even worse than the hold (16.05 versus 15.50~cm).
...
Of the values in the sweep, the one used here, $\lambda = 10^{-4}$, is the most accurate for both movement patterns, and it still leaves an RMSE of 8.1~cm for the fast recordings.
...
For fast movements, the prediction is not effective, with an RMSE of 8.1~cm at $h = 50$~ms and an improvement of only 19\% over not predicting at all at that horizon.
